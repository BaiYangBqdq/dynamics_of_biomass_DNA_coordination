...
%     fATP = NDATP/NT;
%     NTf = max(NT - DNA*Chns,0) ;
%     CTf = NTf/V;
%     CATPf = CTf*fATP;  
    CATPf = max(NDATP - DNA*Chns,0)/V;

    NT = NT + dt*(phiP0*lambda*V/(1+(CATPf/KPD)^n));
    NDATP = NDATP + dt * ((alphal*V + alphad1*Nd1 + alphad2*Nd2) * CDADP/(KD+CDADP) - (beta_datA + beta_rida)*No * CDATP/(KD+CDATP) + phiP0*lambda*V/(1+(CATPf/KPD)^n) );
