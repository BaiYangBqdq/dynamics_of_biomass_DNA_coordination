...
alphaA_list = 2.^[0:0.2:6];
mean_G_conc = nan(size(alphaA_list));
for id = 1:length(alphaA_list)
%% global parameters
DT = 0.5;     %(h)
TT = DT*40 ; % total calculation time (h)
...
%     if t*dt<T_shutdown
        fATP = fATP + dt * ((alphal + alphad1*Nd1/V + alphad2*Nd2/V) * (1-fATP)/(KDP+1-fATP) - (beta_datA + beta_rida) * No/V * fATP/(KDP+fATP) + alphaA*lambda*(1-fATP));
%     else
...
%%
rel_dnaA = alphaA_list/alphaA_list(16);
